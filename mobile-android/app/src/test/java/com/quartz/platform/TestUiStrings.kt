...
class TestUiStrings : UiStrings {
    override fun get(resId: Int, vararg formatArgs: Any): String {
        val template = when (resId) {
            ...
            R.string.error_xfeeder_proximity_not_eligible -> "Mode proximité non activable: rapprochez-vous de la zone cible (≤ 70m)."
            R.string.error_xfeeder_proximity_altitude_unavailable -> "Mode proximité non activable: altitude indisponible ou imprécise."
            R.string.error_xfeeder_proximity_requires_reference_altitude -> "Mode proximité non activable: renseignez l'altitude antenne de référence."
            R.string.error_xfeeder_proximity_reference_altitude_invalid -> "Altitude antenne invalide: utilisez une valeur numérique (mètres)."
            ...
            R.string.error_ret_update_geospatial_context -> "Impossible d'enregistrer le contexte géospatial RET."
            ...
            R.string.error_ret_zone_extension_reason_required -> "Renseignez un motif avant d'étendre la zone RET."
            R.string.error_ret_zone_extension_requires_proximity_off -> "Désactivez le mode proximité RET avant d'étendre la zone."
            R.string.error_ret_proximity_not_eligible -> "Mode proximité RET non activable: rapprochez-vous de la zone cible (≤ 70m)."
            R.string.error_ret_proximity_altitude_unavailable -> "Mode proximité RET non activable: altitude indisponible ou imprécise."
            R.string.error_ret_proximity_requires_reference_altitude -> "Mode proximité RET non activable: renseignez l'altitude antenne de référence."
            R.string.error_ret_proximity_reference_altitude_invalid -> "Altitude antenne RET invalide: utilisez une valeur numérique (mètres)."
            ...
            R.string.info_ret_zone_extension_saved -> "Contexte géospatial RET mis à jour."
            R.string.info_ret_zone_extension_reset -> "Zone RET réinitialisée au rayon par défaut."
            R.string.info_ret_proximity_enabled -> "Mode proximité RET activé pour cette session locale."
            R.string.info_ret_proximity_disabled -> "Mode proximité RET désactivé pour cette session locale."
            R.string.info_ret_zone_extension_max_reached -> "Rayon maximum RET atteint (300m)."
            R.string.info_ret_user_location_unavailable -> "Position utilisateur indisponible pour la session RET."
            else -> "res:$resId"
        }
